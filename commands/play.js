...
module.exports = {
    ...
    async execute(message,args, cmd, client, Discord){
        ...
        if (!voice_channel) return message.channel.send('Aby wykonać to polecenie, musisz być w kanale!');
        ...
        if (cmd === 'play'){
            if (!args.length) return message.channel.send('Musisz wysłać co chcesz posłuchać jako droga fraza!');
            let song = {};
            ...
            if (ytdl.validateURL(args[0])) {
                ...
                if (video){
                    song = { title: video.title, url: video.url }
                     ...
                     message.channel.send('Błąd podczas wyszukiwania filmu.');
                }
            }
            ...
            if (!server_queue){
                ...
                try {
                    ...
                    message.channel.send('Wystąpił błąd podczas łączenia!');
                    throw err;
                }
            } else{
                server_queue.songs.push(song);
                return message.channel.send(`👍 **${song.title}** dodany do kolejki!`);
            }
        }
...
const video_player = async (guild, song) => {
    ...
    await song_queue.text_channel.send(`🎶 Teraz gra **${song.title}**`)
}

const skip_song = (message, server_queue) => {
    if (!message.member.voice.channel) return message.channel.send('Aby wykonać to polecenie, musisz być w kanale!');
    if(!server_queue){
        return message.channel.send(`Brak utworów w kolejce 😔`);
    }
